...
RegisterServerEvent('krisu_housing:enterHouse')
AddEventHandler('krisu_housing:enterHouse', function(id)
    local src = source
    ...
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {['@identifier'] = xPlayer.identifier}, function(result)
        ...
        if house.houseId == id then
            for k, v in pairs(Config.HouseSpawns) do
                if not v['taken'] then
                    TriggerClientEvent('krisu_housing:spawnHouse', xPlayer.source, v['coords'], furniture)
...
RegisterServerEvent('krisu_housing:buy_furniture')
AddEventHandler('krisu_housing:buy_furniture', function(category, id)
...
RegisterServerEvent('krisu_housing:leaveHouse')
AddEventHandler('krisu_housing:leaveHouse', function(house)
    local src = source
...
RegisterServerEvent('krisu_housing:deleteInstance')
AddEventHandler('krisu_housing:deleteInstance', function()
    local src = source
    if instances[src] then
        Config.HouseSpawns[instances[src]['housespawn']]['taken'] = false
        for k, v in pairs(instances[src]['players']) do
            TriggerClientEvent('krisu_housing:leaveHouse', v, instances[src]['id'])
        end
        instances[src] = {}
    end
end)

RegisterServerEvent('krisu_housing:letIn')
AddEventHandler('krisu_housing:letIn', function(plr, storage)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if instances[src] then
        if not instances[src]['players'][plr] then 
            ...
            TriggerClientEvent('krisu_housing:knockAccept', plr, instances[src]['coords'], instances[src]['id'], storage, spawnpos, furniture, src)
        end
    end
end)

RegisterServerEvent('krisu_housing:unKnockDoor')
AddEventHandler('krisu_housing:unKnockDoor', function(id)
    local src = source
    if instances[houses[id]] then
        TriggerClientEvent('krisu_housing:removeDoorKnock', instances[houses[id]]['owner'], src)
    end
end)

RegisterServerEvent('krisu_housing:knockDoor')
AddEventHandler('krisu_housing:knockDoor', function(id)
    local src = source
    if instances[houses[id]] then
        TriggerClientEvent('krisu_housing:knockedDoor', instances[houses[id]]['owner'], src)
    else
        TriggerClientEvent('esx:showNotification', src, Strings['Noone_Home'])
    end
end)

RegisterServerEvent('krisu_housing:setInstanceCoords')
AddEventHandler('krisu_housing:setInstanceCoords', function(coords, housecoords, prop, placedfurniture)
    local src = source
    if instances[src] then
...
RegisterServerEvent('krisu_housing:exitHouse')
AddEventHandler('krisu_housing:exitHouse', function(id)
    local src = source
    if instances[src] then
        for k, v in pairs(instances['players']) do
            TriggerClientEvent('krisu_housing:exitHouse', v, id)
...
RegisterServerEvent('krisu_housing:buyHouse')
AddEventHandler('krisu_housing:buyHouse', function(id)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {['@identifier'] = xPlayer.identifier}, function(result)
        local house = json.decode(result[1].house)
    ...
    Wait(1500)
    TriggerClientEvent('krisu_housing:reloadHouses', -1)
end)

RegisterServerEvent('krisu_housing:sellHouse')
AddEventHandler('krisu_housing:sellHouse', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {['@identifier'] = xPlayer.identifier}, function(result)
        local house = json.decode(result[1].house)
...
RegisterServerEvent('krisu_housing:getOwned')
AddEventHandler('krisu_housing:getOwned', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {['@identifier'] = xPlayer.identifier}, function(result)
        local house = json.decode(result[1].house)
        MySQL.Async.fetchAll("SELECT * FROM bought_houses", {}, function(result)
            TriggerClientEvent('krisu_housing:setHouse', xPlayer.source, house, result)
        end)
    end)
end)

RegisterServerEvent('krisu_housing:furnish')
AddEventHandler('krisu_housing:furnish', function(house, furniture)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
...
ESX.RegisterServerCallback('krisu_housing:hasGuests', function(source, cb)
...
ESX.RegisterServerCallback('krisu_housing:hostOnline', function(source, cb, host)
    local online = false
...
ESX.RegisterServerCallback('krisu_housing:getInventory', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    
    cb({['items'] = xPlayer.inventory, ['weapons'] = xPlayer.getLoadout()})
end)

ESX.RegisterServerCallback('krisu_housing:getHouseInv', function(source, cb, owner)
	local xPlayer = ESX.GetPlayerFromId(source)
    local items, weapons = {}, {}
...
RegisterServerEvent('krisu_housing:withdrawItem')
AddEventHandler('krisu_housing:withdrawItem', function(type, item, amount, owner)
	local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
...
RegisterServerEvent('krisu_housing:storeItem')
AddEventHandler('krisu_housing:storeItem', function(type, item, amount, owner)
	local src = source
	local xPlayer = ESX.GetPlayerFromId(src)
