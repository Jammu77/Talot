...
Citizen.CreateThread(function()
    ...
    if Config.IKEABlip['Enabled'] then
        ...
        AddTextComponentString(('Huonekaluliike'))
...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent('krisu_housing:getOwned')
                            ...
                            ESX.UI.Menu.Open(
                                ...
                                function(data, menu)
                                    if data.current.value == 'enter' then
                                        TriggerServerEvent('krisu_housing:enterHouse', k)
                                        menu.close()
                                    ...
                                    elseif data.current.value == 'sell' then
                                        ...
                                        function(data2, menu2)
                                            if data2.current.value == 'yes' then
                                                TriggerServerEvent('krisu_housing:sellHouse')
                                                ESX.UI.Menu.CloseAll()
                                                Wait(5000)
                                            else
                                                menu2.close()
                            ...
                            if not AvailableHouses[k] and OwnedHouse.houseId == 0 then
                                ...
                                function(data, menu)
                                    if data.current.value == 'yes' then
                                        TriggerServerEvent('krisu_housing:buyHouse', k)
                                        ESX.UI.Menu.CloseAll()
                                        Wait(5000)
                                    else
                                        menu.close()
                ...
                if OwnedHouse.houseId == k then
                    ...
                    if found then
                        while #(GetEntityCoords(PlayerPedId()) - coords) <= 45.0 and IsPedInAnyVehicle(PlayerPedId(), false) and Config.EnableGarage do
                            Wait(0)
                            DrawMarker(25, coords-vector3(0.0, 0.0, 0.5), vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 1.2), 255, 255, 25, 150, false, false, 2, false, false, false)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if dist <= 50.0 then
            DrawMarker(25, Config.Furnituring['enter'], vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 1.0), 255, 0, 255, 150, false, false, 2, false, false, false)
            if dist <= 1.5 then
                    ...
                    while true do
                        ...
                        elseif IsControlJustReleased(0, 191) then
                            ...
                            function(data, menu)
                                menu.close()
                                if data.current.value == 'yes' then
                                    TriggerServerEvent('krisu_housing:buy_furniture', currentcategory, current)
                                    DoScreenFadeOut(250)
                                    Wait(1500)
                                    DoScreenFadeIn(500)
                                end
...
RegisterNetEvent('krisu_housing:spawnHouse')
AddEventHandler('krisu_housing:spawnHouse', function(coords, furniture)
    ...
    TriggerServerEvent('krisu_housing:setInstanceCoords', exit, coords, prop, OwnedHouse['furniture'])
    ...
    while in_house do
        ...
        DrawMarker(25, exit, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 1.0), 255, 0, 255, 150, false, false, 2, false, false, false)
        DrawMarker(25, storage, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 1.0), 255, 0, 255, 150, false, false, 2, false, false, false)
            ...
            if IsControlJustReleased(0, 38) then
                ...
                function(data, menu)
                    if data.current.value == 'exit' then
                        ESX.TriggerServerCallback('krisu_housing:hasGuests', function(has)
                            if not has then
                                ESX.UI.Menu.CloseAll()
                                TriggerServerEvent('krisu_housing:deleteInstance')
                                Wait(3500)
                                in_house = false
                                return
                            else
                        ...
                        ESX.TriggerServerCallback('krisu_housing:hasGuests', function(has)
                            if not has then
                                ...
                                function(data2, menu2)
                                    ...
                                    TriggerServerEvent('krisu_housing:furnish', OwnedHouse, furniture)
                                    menu2.close()
                                    editing = false
                                end, 
                                    function(data2, menu2)
                    ...
                    elseif data.current.value == 'accept' then
                        ...
                        function(data2, menu2)
                            if Knockings[data2.current.value] then
                                TriggerServerEvent('krisu_housing:letIn', data2.current.value, storage)
                            end
                            menu2.close()
                        end, 
                            function(data2, menu2)
...
RegisterNetEvent('krisu_housing:leaveHouse')
...
RegisterNetEvent('krisu_housing:knockAccept')
AddEventHandler('krisu_housing:knockAccept', function(coords, house, storage, spawnpos, furniture, host)
    ...
    while true do
        ...
        if timer <= GetGameTimer() then
            ...
            ESX.TriggerServerCallback('krisu_housing:hostOnline', function(online)
                if not online then
                    delete = true
                end
            end, host)
        ...
        DrawMarker(25, coords, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 1.0), 255, 0, 255, 150, false, false, 2, false, false, false)
        DrawMarker(25, storage, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 1.0), 255, 0, 255, 150, false, false, 2, false, false, false)
        if Vdist2(GetEntityCoords(PlayerPedId()), coords) <= 1.5 then
            ...
            if IsControlJustReleased(0, 38) then
                ...
                DeleteObject(prop)
                DoScreenFadeIn(1500)
                TriggerServerEvent('krisu_housing:leaveHouse', house)
                return
            end
        end
        if Vdist2(GetEntityCoords(PlayerPedId()), storage) <= 2.0 then
...
RegisterNetEvent('krisu_housing:reloadHouses')
AddEventHandler('krisu_housing:reloadHouses', function()
    while ESX == nil do TriggerEvent('esx:getSharedObject', function(obj) ESX = obj end) Wait(0) end
    while ESX.GetPlayerData().job == nil do Wait(0) end
    TriggerServerEvent('krisu_housing:getOwned')
end)

RegisterNetEvent('krisu_housing:knockedDoor')
AddEventHandler('krisu_housing:knockedDoor', function(src)
...
RegisterNetEvent('krisu_housing:removeDoorKnock')
AddEventHandler('krisu_housing:removeDoorKnock', function(src)
    local newTable = {}
    for k, v in pairs(Knockings) do
        if v.value ~= src then
...
RegisterNetEvent('krisu_housing:setHouse')
AddEventHandler('krisu_housing:setHouse', function(house, purchasedHouses)
    OwnedHouse = house
...
HelpText = function(msg, coords)
    if not coords or not Config.Use3DText then
        ...
        DrawText3D(coords, string.gsub(msg, "~INPUT_CONTEXT~", "~b~[~w~E~b~]~w~"), 0.35)
    end
end
